...
function checkParams (done) {
  ...
  // The scope:
  this.scope = body.scope || query.scope;
...
function checkClient (done) {
  ...
  this.model.getClient(this.clientId, null, function (err, client) {
    ...
    // Allow self.redirectUri to be anything localhost or 127.0.0.1
    var parsedUrl = url.parse(self.redirectUri);
...
function redirect (done) {
  this.res.redirect(this.redirectUri + '?code=' + this.authCode +
